   ...
   if ( count ($line) > 2 ) {
      if ( count ($line) != 17 ) { print ( "ERROR Line $recs in Builder $latestCsv is bad got " . count ($line) . "\n"); }
        ...
        if ( isset ( $mapArr[$target] ) ) { // Only if its a tag we are looking for
          ...
          $key = rtrim ( $key, "^" ); // get rid of all trailing ^ chars
      ...
      foreach ( $r_v as $r_k2 => $r_v2 ) {
        ...
        if ( !$hit_m ) {
          if ( anyWordMatch( $b_model, $r_model )) {  // does anything at all match, we got a hit on plan and builder
            print ( "MAYBE: $devName Runway Model [$r_model] Builder Model [ $b_model ] match? Plan $r_plan & Builder $b_builder do match.\n");
          }
        }
